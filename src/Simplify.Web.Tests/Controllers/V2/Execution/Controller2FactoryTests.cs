...
[TestFixture]
public class Controller2FactoryTests
{
	[Test]
	public void CreateController_Controller_PropertiesAssigned()
	{
		...
		var mc = new MatchedController(Mock.Of<IController1Metadata>(m => m.ControllerType == typeof(StringResponseController)), new Dictionary<string, object>());
		...
		var r = (Controller2Base)new Controller2Factory(scope.Resolver).CreateController(mc);
